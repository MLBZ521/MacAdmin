...
function runReports($deviceType, $IDs, $urlID, $csvLocation) {
    ...
    ForEach ($ID in $IDs) {
        ...
        $objectOf_Devices."advanced_${deviceType}_search"."${deviceType}s" | ForEach-Object {$_.$deviceType} | Select-Object -Property $($objectOf_Devices."advanced_${deviceType}_search".display_fields.display_field.name | ForEach-Object { $_ -Replace " ","_" -Replace "-","_"}) | Export-Csv -Path "$csvLocation\${reportName}.csv" -Append -NoTypeInformation
    }
